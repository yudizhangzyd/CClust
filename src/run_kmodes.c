...
int make_results(outres **out, data *dat, options *opt);
void free_results(outres *out);
...
#else
SEXP r_kmodes(SEXP K_r,
	      SEXP datafile_r,
	      SEXP n_init_r,
	      SEXP kmodes_algorithm_r,
	      SEXP init_method_r,
	      SEXP seed_r,
	      SEXP shuffle_r
	      )
{
	...
	srand(opt->seed);
	opt->kmodes_algorithm = *INTEGER(kmodes_algorithm_r);
	opt->init_method = *INTEGER(init_method_r);
	...
	SEXP r_best_seed_idx = PROTECT(allocVector(INTSXP, opt->K));
	memcpy(INTEGER(r_best_seed_idx), out->best_seed_idx, opt->K * sizeof *out->best_seed_idx);
		...
		if (!opt->n_sd_idx && !opt->pfile) {
			...
			if (opt->init_method
				...
				kmodes_init_random_from_set(opt->K,
					...
					seeds, opt->seed_set);
			else if (opt->init_method == KMODES_INIT_RANDOM_SEEDS)
...
int restore_state(data *dat, options *opt)
{
	...
	while (fgets(temp, 80, fp) != NULL) {
		debug_msg(DEBUG_I, fxn_debug, "restore_state: %s\n", temp);
		if (!strncmp(temp, "Best optimized criterion:",	/* 25 */
			...
			strlen("Best modes:"))) {
			found_best_modes = 1;
			debug_msg(DEBUG_I, fxn_debug, "Found \"Best modes:\".\n");
			/* data on newline */
			for (k = 0; k < opt->K; ++k) {
				if (fscan_data_ts(fp, dat->best_modes[k],
					...
					return mmessage(ERROR_MSG,
						FILE_FORMAT_ERROR,
						opt->soln_file);
				debug_msg(DEBUG_I, fxn_debug, "Best modes, read mode %u:\n", k);
				debug_call(DEBUG_I, fxn_debug,
					fprint_data_ts(stderr, dat->best_modes[k], dat->n_coordinates, 0, 1));
			}
	...
	for (i = 1; i < argc; i++) {
		...
		switch(a) {
			case 'c':
				...
				if (errno)
					goto CMDLINE_ERROR;
				debug_msg(MINIMAL, opt->quiet, "true column = %u\n",
					opt->true_column);
				break;
			case 'j':
				opt->estimate_k = 1;
			...
			case 'f':
				...
				if (i + 1 < argc && argv[i+1][0] != '-') {
					...
					debug_msg(QUIET, opt->quiet, "Will write "
						"data, after possible "
...
int estimate_k(data *dat, options *opt)
{
	...
	for (unsigned int l = 0; l < opt->n_k; ++l) {
		...
		debug_msg(DEBUG_I, fxn_debug, "Best solution is %.0f from file "
		...
		for (unsigned i = 0; i < opt->n_bootstrap; ++i) {
			debug_msg(DEBUG_I, fxn_debug, "BOOTSTRAP %u\n", i);

 ...
 */
#define debug_msg_cont(lvl, fxn_dbg_lvl, ...) do {                             \
	if ((lvl) <= (fxn_dbg_lvl) || (lvl) <= global_debug_level)             \
 ...
 */
#ifdef USE_CURSES
#define cc_msg(wp, lvl, dbg_lvl, msg, ...) do {                                \
	if ((lvl) <= (dbg_lvl) || (lvl) <= global_debug_level)                 \
		(!(wp)                                                         \
 ...
 */
#define cc_msg_cont(wp, level, dbg_level, ...) do {                            \
	if ((level) <= (dbg_level) || (level) <= global_debug_level)           \
		(!(wp)                                                         \
		? fprintf(stderr, __VA_ARGS__)                                 \
 ...
 */
#define debug_call(level, fxn_debug_level, fxn_call) do {                      \
	if ((level) <= (fxn_debug_level) || (level) <= global_debug_level)     \
		(fxn_call);                                                    \
} while (0)

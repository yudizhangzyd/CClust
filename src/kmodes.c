 ...
 * @file kmodes.c
 *
 * Implements various k-modes algorithms.
 *
 ...
 * (1997), 1--8.
 *
 * This code also implements Huang's algorithm.
 *
...
/* data allocation */
static size_t allocate_and_compute_nj(data_t **x, unsigned int n, unsigned int p);
static int allocate_and_compute_njc(data_t **x, unsigned int n, unsigned int p, size_t ncat);
static int allocate_nkjc(unsigned int K, unsigned int p, size_t ncat);
static int allocate_hw_memory(unsigned int n, unsigned int p, unsigned int K);
static int reset_nkjc(size_t ***nkjc, unsigned int K, unsigned int p);

/* Hartigan and Wong algorithm */
static void optra(data_t **a, unsigned int m, unsigned int n, data_t **c, unsigned int k, unsigned int *ic1,
	unsigned int *ic2, unsigned int *nc, int *live, int *cd, double *d, unsigned int *indx,
	unsigned int *nj, size_t **nt, size_t ***nkt, data_t **c2, int wgt, int use_qtran);

static void qtran(data_t **a, unsigned int m, unsigned int n, data_t **c, unsigned int k, unsigned int *ic1,
	unsigned int *ic2, unsigned int *nc, int *live, double *d, unsigned int *indx,
...
static double compute_criterion(data_t **x, data_t **seeds, unsigned int *ic, double *criterion, unsigned int K, unsigned int n, unsigned int p, int wght);

/* k-modes initialization routines */
static int kmodes_init_h97(data_t **x, unsigned int n, unsigned int p, unsigned int K, unsigned int k1, int wgt, int rdm, data_t **seeds, unsigned int *sd_idx);
static int kmodes_init_hd17(data_t **x, unsigned int n, unsigned int p, unsigned int K, unsigned int k1, int wgt, data_t **seeds, unsigned int *sd_idx);
static int kmodes_init_av07(data_t **X, unsigned int n, unsigned int p, unsigned int K, unsigned int k1, int weight, data_t **seeds, unsigned int *sd_idx, int greedy);
static int kmodes_init_clb09(data_t **x, unsigned int n, unsigned int p, unsigned int K, unsigned int k1, int wgt, int rdm, data_t **seeds, void *sd_idx,int type);
static int allocate_density (data_t **x, unsigned int n, unsigned int p);
static int allocate_distance (unsigned int n);
...
/* debugging functions */
static int allocate_nkjc_debug(unsigned int K, unsigned int p, size_t ncat);
			...
			for (l = 0; l < k; ++l) {
				...
				if (l != l1) {
					...
					de = debug_cost_to_move(a, m, n, k, ic1,
								 i, l1, l, wgt);
					debug_msg(DEBUG_II, DEBUG_II, "Cost of"
						" moving member %zu from %u to "
						"%u is %.0f\n", i, l1, l, de);
	...
	for (i = 0; i < mp1; i++, istep++, loop_cnt++) {
		...
		if (istep <= live[l1] || istep <= live[l2]) {
			...
			double de = debug_cost_to_move(a, m, n, k, ic1, i, l1,
								l2, wgt);
			debug_msg(DEBUG_II, DEBUG_II, "Cost of moving member "
	...
	unsigned int p, double bcost, size_t **nt, int fxn_debug)
{
	...
	for (unsigned int j = 0; j < p; ++j) {
		if (fxn_debug) {
			debug_msg(fxn_debug, fxn_debug, "%zu (%u):", j, a[j]);
		...
		if (nkt[j][(int) c[j]] > nkt[j][(int) a[j]] + 1) {
			...
			debug_msg(QUIET, fxn_debug, "[1] %.0f\n", de);
		...
		/* (w/ weights): add char that breaks tie (3-4) */
			...
			if (c[j] < a[j]) {
				de += (double) (nt[j][(int) c[j]] + nt[j][(int) a[j]])
					/ (nt[j][(int) c[j]] * nt[j][(int) a[j]]);
				debug_msg(QUIET, fxn_debug, "[2] %.0f\n", de);
				...
				for (unsigned int l = 0; l < __nj[j]; ++l)
					if (l == (unsigned int) c[j])	/* previous mode */
						...
						de += (double) nkt[j][l] *
							...
							- (nt[j][(int) c[j]] + nt[j][l])
							/(nt[j][(int) c[j]] * nt[j][l]));
				debug_msg(QUIET, fxn_debug, "[3] %.0f\n", de);
			}
			...
			for (unsigned int l = 0; l < __nj[j]; ++l)
				if (l == (unsigned int) c[j])	/* previous mode */
					...
					de += (double) nkt[j][l] *
						...
						- (nt[j][(int) c[j]] + nt[j][l])
						/(nt[j][(int) c[j]] * nt[j][l]));
			debug_msg(QUIET, fxn_debug, "[5] %.0f\n", de);
		}
	...
	data_t *c, data_t *c2, unsigned int p, size_t **nt, int fxn_debug)
{
	...
	for (unsigned int j = 0; j < p; ++j) {
		...
		if (a[j] != c[j]) {
			de += (double) (nt[j][(int) c[j]] + nt[j][(int) a[j]])
				/ (nt[j][(int) c[j]] * nt[j][(int) a[j]]);
			debug_msg(QUIET, fxn_debug, "remove non-mode %u "
				"(%.0f)\n", a[j], de);
		...
		/* removing modal character and departure will change mode */
			...
			debug_msg(QUIET, fxn_debug, "remove mode %u, new mode "
				"%u (%.0f)\n", a[j], c2[j], de);
		}
	}
	return de;
	...
	data_t **c2, unsigned int K, unsigned int p)
{
	...
	debug_msg(ABSOLUTE_SILENCE, DEBUG_I, "%s\n", str);
	for (unsigned int k = 0; k < K; ++k) {
		fprintf(stderr, "Cluster %u:", k);
		if (c) fprint_data_ts(stderr, c[k], p, 1, 0);
		if (c2) {
			fprintf(stderr, "; ");
 ...
 */
int
kmodes_init (data_t **x, unsigned int n, unsigned int p, unsigned int k,
	     unsigned int k1, data_t **seeds, unsigned int *sidx, int method,
	     int weight)
{
	if (method == KMODES_INIT_H97) {
 ...
 */
static int
kmodes_init_h97 (data_t **x, unsigned int n, unsigned int p, unsigned int K,
 ...
 */
static int
kmodes_init_hd17 (data_t **x, unsigned int n, unsigned int p, unsigned int K,
 ...
 */
static int
allocate_density (data_t **x, unsigned int n, unsigned int p)
{
 ...
 */
static int
allocate_distance (unsigned int n)
{
	MAKE_1ARRAY(__dis, n);
 ...
 */
static int
kmodes_init_clb09 (data_t **x, unsigned int n, unsigned int p, unsigned int K,
